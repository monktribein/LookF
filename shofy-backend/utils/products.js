...
// ---- Override dataset to use fashion products only ----
const fashionProducts = require("./fashion-products");
const slugify = require("./slugify");

const normalizeCategory = (parent = "") => {
  const val = parent.toLowerCase();
  if (val.includes("t-shirt")) return "tshirts";
  if (val.includes("shirt")) return "shirts";
  if (val.includes("bottom") || val.includes("trouser")) return "bottom-wear";
  return val;
};

const fashionOnly = fashionProducts.map((p, idx) => {
  const mainCategory = p.mainCategory || normalizeCategory(p.parent || "");
  const subCategory =
    p.subCategory ||
    slugify(p.children || "").replace(/t\-shirts?/g, "tshirts");
  const stock = typeof p.stock === "number" ? p.stock : p.quantity || 50;
  return {
    ...p,
    _id: p._id || `65f400000000000000000${100 + idx}`,
    productType: "fashion",
    currency: p.currency || "INR",
    mainCategory,
    subCategory,
    gender: (p.gender || "men").toLowerCase(),
    stock,
    isNewArrival: p.isNewArrival !== false,
  };
});

module.exports = fashionOnly;